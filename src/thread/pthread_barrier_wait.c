...
int pthread_barrier_wait(pthread_barrier_t *b)
{
	...
	if (!inst) {
		...
		while (inst->finished == 1)
			__hq_raw_syscall4(SYS_futex, (unsigned long)&inst->finished, FUTEX_WAIT|FUTEX_PRIVATE, 1, 0) != -ENOSYS
			|| __hq_raw_syscall4(SYS_futex, (unsigned long)&inst->finished, FUTEX_WAIT, 1, (unsigned long)0);
